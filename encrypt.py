...
def encrypt_file(input_file, public_key):
    """Encrypts a file and measures encryption time and memory usage."""
    kem = oqs.KeyEncapsulation(KYBER_ALG)
    ...
    with open(enc_file, "wb") as f:
        f.write(len(ciphertext).to_bytes(4, "big") + ciphertext + encrypted_data)
    end_time = time.time()
...
def decrypt_file(encrypted_file, secret_key, kem):
    """Decrypts a file and measures decryption time and memory usage."""
